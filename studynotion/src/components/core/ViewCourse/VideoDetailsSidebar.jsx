...
export default function VideoDetailsSidebar({ setReviewModal }){
  ...
  const {courseSectionData, courseEntireData, totalNoOfLectures, completedVideos=[]} = useSelector((state) => state.viewCourse)
  ...
  console.log("completedlecture",completedVideos);
              ...
              {activeStatus === course?._id && (
                <div className="transition-[height] duration-500 ease-in-out">
                  {course.subSection.map((topic, i) => (
                    ...
                    >
                      <input  type="checkbox" checked={completedVideos.includes(topic?._id)} onChange={() => {}} />
                      {topic.title}
                    </div>
                  ))}
                </div>
