...
//         <Swiper className="w-full " slidesPerView={3} spaceBetween={500} loop={true} freeMode={true} autoplay={{delay: 2500,  disableOnInteraction: false, }} >
...
//                 </div> */}
//                 <div className="flex flex-col gap-3 bg-richblack-800 p-3 text-[14px] text-richblack-25">
//   <div className="flex items-center gap-4"> {/* This div ensures that user image, name, and course name are horizontally aligned */}
...
//     <div className="flex flex-col">
//       <h1 className="font-semibold text-richblack-5 flex flex-row">{`${review?.user?.firstName} ${review?.user?.lastName}`}</h1>
//       <h2 className="text-[12px] font-medium text-richblack-500">{review?.course?.courseName}</h2>
//     </div>
//   </div>
//   <p className="font-medium text-richblack-25">
//     {review?.review.split(" ").length > truncateWords ? `${review?.review.split(" ").slice(0, truncateWords).join(" ")} ...` : `${review?.review}`}
//   </p>
//   <div className="flex items-center gap-2"> {/* This div ensures that rating value and stars are horizontally aligned */}
//     <h3 className="font-semibold text-yellow-100">{review.rating.toFixed(1)}</h3>
//     <ReactStars count={5} value={review.rating} size={20} edit={false} activeColor="#ffd700" emptyIcon={<FaStar />} fullIcon={<FaStar />} />
//   </div>
// </div>
